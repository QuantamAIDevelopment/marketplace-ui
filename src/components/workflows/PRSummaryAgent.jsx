            ...
            {response && !isSubmitting && (
              ...
              >
                ...
                {(response.pull_request_url || (response._links && response._links.pull_request && response._links.pull_request.href)) && (
                  <div className="mb-2">
                    <b>PR Link:</b> <a href={response.pull_request_url.replace('api.github.com/repos', 'github.com').replace('/pulls/', '/pull/') || response._links?.pull_request?.href} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">Open PR</a>
                  </div>
                )}
