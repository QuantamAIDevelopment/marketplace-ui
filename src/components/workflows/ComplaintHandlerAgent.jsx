...
const ComplaintHandlerAgent = ({ compact = false }) => {
  ...
  return (
    <div className={compact ? "space-y-4 w-full overflow-hidden" : "space-y-8 w-full"}>
      <div className={compact ? "flex items-center space-x-2 mb-1" : "flex items-center space-x-4 mb-2"}>
        ...
        <h3 className={compact ? "text-lg font-display text-anthropic-dark font-bold" : "text-2xl font-display text-anthropic-dark font-bold"}>Complaint Handler Agent</h3>
      </div>
      <p className={compact ? "text-xs text-gray-600" : "text-sm text-gray-700"}>
        Submit and track customer complaints. Automatically classify, route, and receive status updates via email.
      </p>
      <motion.button
        whileHover={{ scale: 1.05, boxShadow: '0 8px 32px 0 #e5737333' }}
        whileTap={{ scale: 0.97 }}
        onClick={() => navigate('/complaint-handler')}
