...
import { FaUserAlt, FaSync, FaCheckCircle } from 'react-icons/fa';
import { useNavigate } from 'react-router-dom';

const CandidateDetailsSync = ({ compact = false }) => {
  ...
  const statList = [
    { title: 'Synced Profiles', value: stats.syncedProfiles, icon: FaCheckCircle, color: 'bg-green-500' },
    { title: 'Pending Sync', value: stats.pendingSync, icon: FaSync, color: 'bg-yellow-500' },
    { title: 'Failed Sync', value: stats.failedSync, icon: FaUserAlt, color: 'bg-red-500' },
  ];
  ...
  return (
    <div className={compact ? "space-y-4 w-full overflow-hidden" : "space-y-8 w-full"}>
      <div className={compact ? "flex items-center space-x-2 mb-1" : "flex items-center space-x-4 mb-2"}>
        ...
        <h3 className={compact ? "text-lg font-display text-anthropic-dark font-bold" : "text-2xl font-display text-anthropic-dark font-bold"}>Candidate Details Sync</h3>
      </div>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.97 }}
        onClick={() => navigate('/workflows/candidate-details-sync')}
...
export default CandidateDetailsSync; 